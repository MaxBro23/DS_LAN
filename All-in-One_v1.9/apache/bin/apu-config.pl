#!C:\Users\Admin\Desktop\All-in-One_v1.9\perl\bin\perl.exe
use strict;
use warnings;
use Getopt::Long;
...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache\include];
my ${exec_prefix} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache];
my ${datadir} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache\lib];
my ${APRUTIL_DOTTED_VERSION} = q[1.2.3];
my ${CFLAGS} = q[ /nologo /MD /W3 /O2 /D WIN32 /D _WINDOWS /D NDEBUG ];
my ${SHELL} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\WINDOWS\system32\cmd.exe];
my ${CPPFLAGS} = q[];
my ${EXTRA_INCLUDES} = q[];
my ${APRUTIL_SOURCE_DIR} = q[];
my ${prefix} = q[C:\Users\Admin\Desktop\All-in-One_v1.9\apache];
