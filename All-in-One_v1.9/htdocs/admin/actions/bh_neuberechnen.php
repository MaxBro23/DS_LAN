 ...
 * Thread-Url 
 */
$tpl->assign('version', $version = '0.3');
...
class bh_neuberechnen {
    ...
    /*
     * Units that are built at the moment
     */
    function units_build($village_info)
    {
        $units_build_sql = $this->db->query('SELECT `unit`, `num_unit`, `num_finished` FROM `recruit` WHERE `villageid` = '.$village_info['id']);
        while($row = $this->db->fetch($units_build_sql))
        {
          	$units_build_array[] = $row;
        }
        if (count($units_build_array) > 0)
        {
          foreach ($units_build_array as $key => $value)
          {
            $village_info['all_'.$value['unit']] += ($value['num_unit'] - $value['num_finished']);
          }
        }
        return $village_info;
    }
    ...
    /*
     * Buildings that are built at the moment
     */
    function buildings_build($village_info)
    {
        $buildings_build_sql = $this->db->query('SELECT `building` FROM `build` WHERE `villageid` = '.$village_info['id']);
        while($row = $this->db->fetch($buildings_build_sql))
        {
          	$buildings_build_array[] = $row;
        }
        if (count($buildings_build_array) > 0)
        {
          foreach ($buildings_build_array as $key => $value)
          {
            $village_info[$value['building']] += 1;
          }
        }
        return $village_info;
    }
...
if (isset($_GET['start'])) {
  ...
  for ($c = 0; $c < $bh_neuberechnen->doerfer_anzahl; ++$c) {
     ...
     * Units that are built at the moment
     */
    $dorf = $bh_neuberechnen->units_build($dorf);
    /*
     * Buildings that are built at the moment
     */
    $dorf = $bh_neuberechnen->buildings_build($dorf);
    /*
