<h2>{$lang->grab("admin_startpage", "startpage")}</h2>
{if !empty($error)}
...
<form method="POST" action="index.php?screen=startpage&action=add" onSubmit="this.submit.disabled=true;">
	<table class="vis" width="450">
		<tr>
			<th colspan="2">{$lang->grab("admin_startpage", "add_announcement")}</th>
		</tr>
		<tr>
			<td width="50">{$lang->grab("admin_startpage", "text")}:</td>
			<td><textarea cols="45" rows="3" name="text"></textarea></td>
		</tr>
		<tr>
			<td>{$lang->grab("admin_startpage", "link")}:</td>
			<td><input type="text" name="link" size="70"></td>
		</tr>
		<tr>
			<td>{$lang->grab("admin_startpage", "graphic")}:</td>
			<td>
				<table cellspacing="0">
					<tr>
						<td>
		...
		<tr>
			<td align="right" colspan="2"><input type="submit" name="submit" value="{$lang->grab("admin_startpage", "add_announcement")}"></td>
		</tr>
	</table>
</form>
<br />
<table class="vis" width="450">
	<tr>
		<th colspan="2">{$lang->grab("admin_startpage", "already_announced")}</th>
	</tr>
				...
				<table width="100%" cellpadding="0" cellspacing="0">
					<tr>
						{if !empty($announcement.$f_id.link)}
							<td align="left" style="font-size: xx-small;"><a href="{$announcement.$f_id.link}">&raquo; {$lang->grab("admin_startpage", "more")}</a></td>
						{/if}
						<td align="right" style="font-size: xx-small;">{$announcement.$f_id.time}</td>
					</tr>
				</table>
			</td>
			<td><a href="index.php?screen=startpage&action=drop&id={$announcement.$f_id.id}">{$lang->grab("admin_startpage", "delete")}</td>
		</tr>
	{/foreach}
</table>
<br /><br />
...
<table class="vis">
	<tr>
		<th colspan="2">{$lang->grab("admin_startpage", "close_login")}</th>
	</tr>
	<tr>
		<td>{$lang->grab("admin_startpage", "close_login")}?</td>
		<td><input type="radio" name="login_locked" value="yes" {if $login.login_locked=='yes'}checked="checked"{/if}> {$lang->grab("admin_startpage", "closed")}<br /><input type="radio" name="login_locked" value="no" {if $login.login_locked=='no'}checked="checked"{/if}>{$lang->grab("admin_startpage", "not_closed")}</td>
	</tr>
	<tr>
