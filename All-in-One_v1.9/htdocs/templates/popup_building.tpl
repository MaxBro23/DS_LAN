...
<p>{$lang->grab("popup_building", "max_stage")}: <b>{$cl_builds->get_maxstage($building)}</b></p>

{if count($cl_builds->get_needed_by_dbname($building))>0}
	<table class="vis"><tr><th colspan="3">{$lang->grab("popup_building", "requirements")}</th></tr>
		<tr>
			{foreach from=$cl_builds->get_needed_by_dbname($building) key=n_building item=n_stage}
				<td><a href="popup_building.php?building={$n_building}">{$cl_builds->get_name($n_building)}</a> ({$lang->grab("popup_building", "stage")} {$n_stage})</td>
			{/foreach}
		</tr>
	</table>
{/if}
...
<table class="vis">
<tr>
<th>{$lang->grab("popup_building", "stage")}</th><th width="220">{$lang->grab("popup_building", "need")}</th><th width="110">{$lang->grab("popup_building", "worker")}</th></tr>
