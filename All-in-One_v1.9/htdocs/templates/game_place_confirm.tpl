{if $type=="attack"}
	<h2>{$lang->get("attack")}</h2>
{else}
	<h2>{$lang->get("support")}</h2>
{/if}
...
<form action="game.php?village={$village.id}&amp;screen=place&amp;action=command&amp;h={$hkey}" method="post" onSubmit="this.submit.disabled=true;">
	...
	<table class="vis" width="300">
		<tr><th colspan="2">{$lang->get("command")}</th></tr>
		<tr><td>{$lang->get("target")}:</td><td><a href="game.php?village={$village.id}&amp;screen=info_village&amp;id={$info_village.id}">{$info_village.name} ({$values.x}|{$values.y}) K{$info_village.continent}</a></td></tr>
		...
		<tr><td>{$lang->get("duration")}:</td><td>{$unit_runtime|format_time}</td></tr>
		<tr><td>{$lang->get("arrival")}:</td><td>{$arrival|format_date}</td></tr>
	</table>
	<br>
	<table class="vis">
		<tr>
	...
	{if $send_units.unit_catapult>0 && $type!='support'}
	    <table class="vis">
	        <tr>
	            <th>{$lang->get("attack_building")}:</th>
	            <td>
