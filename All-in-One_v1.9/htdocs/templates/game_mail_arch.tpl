{if empty($view)}
	...
	{/if}
	<tr><th>{$lang->get("subject")}</th><th width="160">{$lang->get("from")}</th><th width="160">{$lang->get("to")}</th><th width="140">{$lang->get("sent")}</th></tr>
		...
		{/foreach}
		{if count($mails)>0}
			<tr><th><input name="all" type="checkbox" onclick="selectAll(this.form, this.checked)">{$lang->get("sel_all")}</th><th colspan="3"></th></tr>
		{/if}
	</table>
...
{else}
	{if empty($error)}
		...
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=arch">{$lang->get("archivize")}</a></td>
